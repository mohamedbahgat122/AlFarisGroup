-- Replace invalid max(uuid) aggregates in the order shift-change RPCs.
-- The count check remains authoritative; the UUID is selected only after it
-- confirms there is at most one current assignment.
