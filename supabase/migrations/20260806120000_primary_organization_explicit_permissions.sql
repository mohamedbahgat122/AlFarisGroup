-- Treat profiles.home_organization_id as the default organization only.
-- Access and permissions, including for the primary organization, come from
-- explicit organization_access and organization_user_permissions rows.
    ...
    where oa.user_id = p_target_user_id
      and (
        ...
        or not exists (
          ...
          where (item ->> 'organizationId')::uuid = oa.organization_id
        )
      )
  loop
    ...
    values (
      ...
      jsonb_build_object('access_level', null)
    );
  end loop;
...
revoke all on function public.organization_permission_keys() from public, anon;
revoke all on function public.view_only_organization_permission_keys() from public, anon;
...
grant execute on function public.organization_permission_keys() to authenticated, service_role;
grant execute on function public.view_only_organization_permission_keys() to authenticated, service_role;
