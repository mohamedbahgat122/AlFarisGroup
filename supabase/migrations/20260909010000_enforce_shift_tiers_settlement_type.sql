...
alter function public.move_organization_shift_driver(uuid, uuid, uuid, uuid, date, date)
  rename to move_organization_shift_driver_unchecked;
...
begin
  if v_actor_id is null
     or not public.has_organization_permission(v_actor_id, p_organization_id, 'shifts.assign') then
  ...
  if v_settlement_type is distinct from 'tiers'::public.driver_settlement_type then
    return jsonb_build_object('success', false, 'error', 'DRIVER_NOT_TIERS');
  end if;

  -- Keep move operations deterministic when two moves cross the same shifts.
  perform 1
  from public.organization_shift_templates s
  where s.organization_id = p_organization_id
    and s.id in (p_source_shift_id, p_target_shift_id)
  order by s.id
  for update;

  return public.move_organization_shift_driver_unchecked(
    p_organization_id,
    p_source_shift_id,
    p_target_shift_id,
    p_driver_id,
...
grant execute on function public.move_organization_shift_driver_unchecked(uuid, uuid, uuid, uuid, date, date)
  to service_role;
...
begin
  ...
  where organization_id = p_organization_id
    ...
    and settlement_type = 'tiers'::public.driver_settlement_type;
...
-- Protect all normal-shift assignment writes, including the existing move RPC
-- and any future service-side write path, from accepting per-order drivers.
create or replace function public.enforce_normal_shift_tiers_driver()
returns trigger
language plpgsql
security definer
set search_path = ''
...
begin
  ...
  if v_settlement_type is distinct from 'tiers'::public.driver_settlement_type then
    raise exception using errcode = 'P0001', message = 'DRIVER_NOT_TIERS';
  end if;
...
create trigger enforce_normal_shift_tiers_driver
  before insert or update on public.organization_shift_assignments
  for each row
  execute function public.enforce_normal_shift_tiers_driver();

revoke all on function public.enforce_normal_shift_tiers_driver()
  from public, anon, authenticated;
