...
-- Order opening eligibility is defined relative to shift start.  The
-- minimum-work value remains an attendance/End policy, not an opening gate.
...
begin
  ...
  for v_date in select v_today union all select v_today - 1 loop
    ...
    if found and now() >= v_start - make_interval(mins => v_policy.open_before_minutes)
       and now() < v_start + make_interval(mins => v_policy.close_after_minutes) then
      return query select v_date, v_start, v_end,
        v_start - make_interval(mins => v_policy.open_before_minutes),
        v_start + make_interval(mins => v_policy.close_after_minutes), v_assignment;
      return;
    end if;
  end loop;
...
alter table public.driver_shifts
  add constraint driver_shifts_attendance_snapshot_shape check (
    ...
    or (
      ...
      and (
        (shift_template_id is not null and order_period_template_id is null
...
begin
  ...
  if not found or v_policy.open_before_minutes is null or v_policy.close_after_minutes is null then raise exception 'ORDER_PERIOD_OPERATIONAL_POLICY_UNCONFIGURED' using errcode = '22023'; end if;
  ...
  if not found and (v_now < v_occ.opens_at or v_now >= v_occ.closes_at) then raise exception 'ORDER_PERIOD_START_WINDOW_NOT_OPEN' using errcode = '42501'; end if;
  ...
  values (v_driver.id, v_driver.organization_id, v_vehicle.id, v_plate, 'open', 1, null, v_template.id, v_occ.scheduled_business_date, v_policy.open_before_minutes, v_policy.minimum_work_minutes, v_now, p_odometer_reading, p_photo_path, p_photo_captured_at) returning * into v_shift;
...
begin
  ...
  if not found or v_policy.open_before_minutes is null or v_policy.close_after_minutes is null then return jsonb_build_object('success', true, 'state', 'policy_unconfigured', 'template_id', v_template.id, 'template_name', v_template.name, 'reason_code', 'ORDER_PERIOD_OPERATIONAL_POLICY_UNCONFIGURED'); end if;
  select * into v_occ from public.resolve_order_period_occurrence(v_driver.organization_id, v_template.id, v_driver.id);
  ...
  return jsonb_build_object('success', true, 'state', case when v_now >= v_occ.opens_at and v_now < v_occ.closes_at then case when found then 'manually_opened' else 'open' end when found then 'manually_opened' else 'before_open_window' end, 'server_now', v_now, 'template_id', v_template.id, 'template_name', v_template.name, 'scheduled_business_date', v_occ.scheduled_business_date, 'scheduled_start_at', v_occ.scheduled_start_at, 'scheduled_end_at', v_occ.scheduled_end_at, 'opens_at', v_occ.opens_at, 'closes_at', v_occ.closes_at, 'is_open_now', (v_now >= v_occ.opens_at and v_now < v_occ.closes_at) or found, 'manual_override_active', found, 'reason_code', null);
end;
$$;
...
begin
  ...
  if v_shift.applied_minimum_work_minutes is not null then
