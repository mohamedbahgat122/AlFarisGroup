-- Migration for V2 Driver RPCs

CREATE OR REPLACE FUNCTION public.create_driver_record_v2(p_actor_user_id uuid, p_driver_id uuid, p_organization_id uuid, p_full_name text, p_nationality text, p_mobile_number text, p_vehicle_type driver_vehicle_type, p_vehicle_number text, p_keeta_username text, p_keeta_driver_id text, p_is_company_sponsored boolean, p_is_vehicle_owner boolean, p_settlement_type driver_settlement_type, p_iqama_number text, p_iqama_expiry_date date, p_driving_license_number text, p_driving_license_expiry_date date, p_driver_card_number text, p_driver_card_expiry_date date, p_iban text, p_bank_name text, p_account_number text, p_documents jsonb, p_vehicle_id uuid DEFAULT NULL::uuid, p_nfc_number text DEFAULT NULL::text)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
BEGIN
  ...
  INSERT INTO public.drivers (
    ...
    vehicle_id, vehicle_number, keeta_username, keeta_driver_id, is_company_sponsored,
    ...
    driver_card_expiry_date, nfc_number, status, created_by_user_id,
    updated_by_user_id, deleted_at, deleted_by_user_id
  )
  VALUES (
    p_driver_id, p_organization_id, v_full_name, v_nationality, v_mobile_number,
    p_vehicle_type, p_vehicle_id, v_vehicle_number, v_keeta_username,
...
REVOKE ALL ON FUNCTION public.create_driver_record_v2 FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.create_driver_record_v2 TO authenticated, service_role;

CREATE OR REPLACE FUNCTION public.update_driver_record_v2(p_actor_user_id uuid, p_driver_id uuid, p_organization_id uuid, p_full_name text, p_nationality text, p_mobile_number text, p_vehicle_type driver_vehicle_type, p_vehicle_number text, p_keeta_username text, p_keeta_driver_id text, p_is_company_sponsored boolean, p_is_vehicle_owner boolean, p_settlement_type driver_settlement_type, p_iqama_number text, p_iqama_expiry_date date, p_driving_license_number text, p_driving_license_expiry_date date, p_driver_card_number text, p_driver_card_expiry_date date, p_iban text, p_bank_name text, p_account_number text, p_documents jsonb DEFAULT '[]'::jsonb, p_vehicle_id uuid DEFAULT NULL::uuid, p_nfc_number text DEFAULT NULL::text)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
