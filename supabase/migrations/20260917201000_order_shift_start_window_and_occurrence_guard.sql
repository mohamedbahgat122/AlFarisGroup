...
-- Bound Order Shift Start to the resolved occurrence and prevent duplicate
-- attendance for an already-created driver/template/business-date occurrence.
...
begin
  ...
  where id = v_selected_template_id
    and organization_id = v_driver.organization_id
  for update;
  if not found then
    raise exception 'ORDER_PERIOD_DRIVER_NOT_ASSIGNED' using errcode = '42501';
  end if;
  ...
  for update;
  if not found or v_policy.open_before_minutes is null then
    raise exception 'ORDER_PERIOD_OPERATIONAL_POLICY_UNCONFIGURED' using errcode = '22023';
  end if;

  -- Serialize all attendance starts for this driver. This protects the
  -- occurrence check without imposing a new constraint on historical rows.
  ...
  for update;
  if not found
     or v_locked_driver.organization_id <> v_driver.organization_id
  ...
  if not found then
    raise exception 'ORDER_PERIOD_NO_CURRENT_OCCURRENCE' using errcode = '42501';
  end if;
  ...
  if exists (
    ...
    raise exception 'ORDER_PERIOD_ATTENDANCE_ALREADY_EXISTS' using errcode = 'P0001';
  end if;
  ...
  if not found and v_now < v_occ.opens_at then
    raise exception 'ORDER_PERIOD_START_WINDOW_NOT_OPEN' using errcode = '42501';
  end if;

  select fv.* into v_vehicle
  ...
  insert into public.driver_shifts (
    ...
    scheduled_business_date, applied_start_open_before_minutes,
    applied_end_before_minutes, applied_minimum_work_minutes, started_at,
    start_odometer_reading, start_photo_path, start_photo_captured_at
  ) values (
    v_driver.id, v_driver.organization_id, v_vehicle.id, v_plate, 'open', 1,
    null, v_template.id, v_occ.scheduled_business_date,
    coalesce(v_policy.open_before_minutes, 0), v_policy.end_before_minutes,
    v_policy.minimum_work_minutes, v_now, p_odometer_reading, p_photo_path,
    p_photo_captured_at
  ...
  insert into public.activity_logs (
    ...
    v_user_id, v_driver.organization_id, 'driver_order_shift_started',
    'driver_shift', v_shift.id,
    jsonb_build_object(
      'driver_id', v_driver.id,
      'order_period_template_id', v_template.id,
  ...
  return jsonb_build_object(
    ...
    'scheduled_business_date', v_shift.scheduled_business_date,
    'applied_end_before_minutes', v_shift.applied_end_before_minutes,
...
begin
  ...
  if exists (
    ...
    return jsonb_build_object('success', true, 'state', 'closed', 'server_now', v_now, 'template_id', v_template.id, 'template_name', v_template.name, 'scheduled_business_date', v_occ.scheduled_business_date, 'scheduled_start_at', v_occ.scheduled_start_at, 'scheduled_end_at', v_occ.scheduled_end_at, 'opens_at', v_occ.opens_at, 'closes_at', v_occ.closes_at, 'earliest_start_at', v_occ.opens_at, 'auto_close_at', v_occ.closes_at, 'is_open_now', false, 'manual_override_active', false, 'reason_code', 'ORDER_PERIOD_ATTENDANCE_ALREADY_EXISTS');
  end if;
  ...
  return jsonb_build_object(
    'success', true,
    'state', case
      when v_now >= v_occ.scheduled_end_at then 'closed'
    ...
    'is_open_now', v_now < v_occ.scheduled_end_at and (v_now >= v_occ.opens_at or found),
    'manual_override_active', v_now < v_occ.scheduled_end_at and found,
    'reason_code', null
  );
end;
$$;
...
begin
  ...
  insert into public.organization_order_period_open_overrides (
    ...
    p_organization_id, p_order_period_template_id, p_driver_id,
    v_occ.scheduled_business_date, v_now, v_occ.scheduled_end_at, v_actor
  )
